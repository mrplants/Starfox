define(['text!lib/constants/baseURL.const'], function(baseURL) {
  ...
  return function() {
    ...
    function requestObjFile() {
      ...
      objRequest.open('GET', self.origin + 'game/3D/' + self.location + '/' + self.location + '.obj', true);
      objRequest.send();
    }
    ...
    function requestMtlFile() {
      ...
      objRequest.open('GET', self.origin + 'game/3D/' + self.location + '/' + self.location + '.mtl', true);
      objRequest.send();
    }

define(['ObjParser', 'Matrix', 'MatrixStack'], function(ObjParser, Matrix, MatrixStack) {
	...
	return function Model(gl, modelName, callback) {
        ...
        self.shininess = 1.0;
        self.baseColor = [0.3,0.3,0.3,1.0];
        ...
        */
        self.draw = function draw(perspectiveProjectionStack, program, optionalTempModelMatrix) {
            ...
            if (optionalTempModelMatrix !== undefined) {
                var modelUniform = gl.getUniformLocation(program, 'modelTransform');
                gl.uniformMatrix4fv(modelUniform, false, self.modelView.multiply(optionalTempModelMatrix).webGLFormat());
            } else {
                var modelViewUniform = gl.getUniformLocation(program, 'modelTransform');
                gl.uniformMatrix4fv(modelViewUniform, false, self.modelView.webGLFormat());
            }
            var worldUniform = gl.getUniformLocation(program, 'worldTransform');
            gl.uniformMatrix4fv(worldUniform, false, perspectiveProjectionStack.getProduct().webGLFormat());
        ...
        if (modelName !== undefined) {
            self.parser.setLocation(modelName, function() {
                ...
                far = self.parser.maxes.z;
                near = self.parser.mins.z;
                ...
                var sideLength = Math.max(Math.max(Math.abs(right-left), Math.abs(top-bottom)), Math.abs(far-near));

                self.modelView.orthogonize(centerZ+sideLength/2, centerZ-sideLength/2, centerY+sideLength/2, centerY-sideLength/2, centerX+sideLength/2, centerX-sideLength/2);
